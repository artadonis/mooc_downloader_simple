...
public class DownloadFile extends Thread {
    ...
    public void run() {
        while (endPos > startPos && !isDownloadOver) {
            try {
                ...
                //输出log信息
                logger.info("开始 " + threadId + "：" + property + endPos);
                //printHeader(conn);
                ...
                logger.info("#over#Thread: " + threadId + ", startPos: " + startPos + ", endPos: " + endPos);
                logger.info("Thread " + threadId + " is execute over!");
        ...
        if (endPos < startPos && !isDownloadOver) {
            logger.info("Thread " + threadId  + " startPos > endPos, not need download file !");
            this.isDownloadOver = true;
        }
        if (endPos == startPos && !isDownloadOver) {
            logger.info("Thread " + threadId  + " startPos = endPos, not need download file !");
    ...
    public static void printHeader(URLConnection conn) {
        ...
        while (true) {
            ...
            i++;
            if (header != null) {
            	logger.info(header + ":" + conn.getHeaderField(i));
            } else {
                break;
            }
        }
